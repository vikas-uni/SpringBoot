...
public class PersonController {
    ...
    public Flux<Person> findPersonsStreamBackPressure() {
    	LOGGER.info("starting findPersonsStreamBackPressure...");
    	
    	Flux<Person> flux = Flux.fromStream(this::prepareStream).delayElements(Duration.ofMillis(100))
        .doOnNext(person -> LOGGER.info("Server produces: {}", person));
    	
    	LOGGER.info("ending findPersonsStreamBackPressure.");
        return flux;
    }
